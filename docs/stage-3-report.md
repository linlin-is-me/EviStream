...
The offline gate migrates an empty disposable database through Stage 2 and Stage 3,
downgrades to Stage 2, upgrades again, runs Mock Embedding and executes the full Python
test suite. CI performs the same gate without credentials or external network calls.
...
Local WSL2 verification on 2026-09-01 completed with 62 Python tests passing and 87.25%
coverage. Ruff, strict mypy, the disposable migration cycle, frontend lint, two Vitest
tests and the production frontend build also passed.
